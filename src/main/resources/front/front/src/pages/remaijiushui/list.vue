...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"20px 10px","margin":"0px auto 0","borderColor":"#00adb5","alignItems":"center","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"#f5f5f5","borderWidth":"0px 0 0px 0","width":"100%","borderStyle":"dashed","height":"auto","order":"2"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>酒水名称</div>
        <el-input v-model="formSearch.jiushuimingcheng" placeholder="酒水名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>包装形式</div>
        <el-select v-model="formSearch.baozhuangxingshi" placeholder="请选择包装形式" :clearable="true">
          <el-option v-for="(item, index) in baozhuangxingshiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>生产年份</div>
        <el-input v-model="formSearch.shengchannianfen" placeholder="生产年份" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	  ...
	  <el-button v-if="isAuth('remaijiushui','新增')" :style='{"cursor":"pointer","border":"0px solid #db961f","padding":"0px 15px","boxShadow":"0px 0px 0px #f8a412","margin":"0px 10px 0 0","color":"#fff","outline":"none","borderRadius":"4px","background":"linear-gradient(320deg, rgba(61,199,196,1) 0%, rgba(152,247,245,1) 80%, rgba(61,199,196,1) 100%),rgb(61,199,196)","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/remaijiushuiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"padding":"0 0px","margin":"20px 0 0","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"border":"1px solid #eee","cursor":"pointer","padding":"10px","boxShadow":"0px 0px 4px #eee","margin":"0 0 20px","display":"flex","justifyContent":"space-between","borderRadius":"4px","flexWrap":"wrap","background":"rgba(255,255,255,1)","width":"49%","fontSize":"0","position":"relative","height":"240px"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div class="item-info" :style='{"width":"48%","padding":"0px 10px","overflow":"hidden","display":"inline-block","height":"100%"}'>
					<div :style='{"padding":"0 10px","borderColor":"#9dcde9","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","background":"none","borderWidth":"0 0 2px","lineHeight":"40px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dotted"}' class="name ">{{item.jiushuimingcheng}}</div>
					<div :style='{"padding":"0 10px","borderColor":"#9dcde9","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","background":"none","borderWidth":"0 0 2px","lineHeight":"40px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dotted"}' class="name ">{{item.pinpai}}</div>
					<div :style='{"padding":"0 10px","borderColor":"#9dcde9","margin":"0 0 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","background":"none","borderWidth":"0 0 2px","lineHeight":"40px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dotted"}' class="name ">{{item.chandi}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
        this.$http.get('option/leixing/jiushuileixing').then(res => {
          if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.jiushuimingcheng = '%' + this.indexQueryCondition + '%';
          this.indexQueryCondition = '';
        }
        if (this.formSearch.jiushuimingcheng != '') searchWhere.jiushuimingcheng = '%' + this.formSearch.jiushuimingcheng + '%';
        if (this.formSearch.baozhuangxingshi != '') searchWhere.baozhuangxingshi = this.formSearch.baozhuangxingshi;
        if (this.formSearch.shengchannianfen != '') searchWhere.shengchannianfen = '%' + this.formSearch.shengchannianfen + '%';
        ...
        if (this.curFenlei != '全部') searchWhere.jiushuileixing = this.curFenlei;
        this.$http.get('remaijiushui/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/remaijiushuiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
